...
# Last Modified:  Monday, 4th January 2021 1:29:10 pm
...
def getNonConflictingImgsLablels(imagePath,
    ...
    fixedtraget=[]
    # reconstruction
    # fixeDelta[0,:,:] = torch.zeros(SampleImgNum,n_classes)
    for index in range(n_classes):
        tmp=fixedLabel.clone()*2-1
        for index1 in range(SampleImgNum):

            if fixedLabel[index1,index] == 0:
                fixeDelta[index+1,index1,index] = 1
                tmp[index1,index]             = 1
                ...
                # if index == 0 and fixedLabel[index1,1]==1: # from non-blad to blad, so we need to remove bangs in the same time
                #     fixeDelta[index,index1,1] = -1
                if index == 0:
                    # if bald
                    if fixedLabel[index1,1] == 1:
                        # if bangs
                        fixeDelta[index+1,index1,1] = -1 # set bangs to non bangs
                if index ==1:
                    # if bangs
                    if fixedLabel[index1,0] == 1:
                        # if bald
                        fixeDelta[index+1,index1,0] = -1 # set bald to non bald
                if index == 2:
                    if fixedLabel[index1,3] == 1:
                        tmp[index1,3]             = -1
                        fixeDelta[index+1,index1,3] = -1
                    if fixedLabel[index1,4] == 1:
                        tmp[index1,4]             = -1
                        fixeDelta[index+1,index1,4] = -1
                elif index == 3:
                    if fixedLabel[index1,2] == 1:
                        tmp[index1,2]             = -1
                        fixeDelta[index+1,index1,2] = -1
                    if fixedLabel[index1,4] == 1:
                        tmp[index1,4]             = -1
                        fixeDelta[index+1,index1,4] = -1
                elif index == 4:
                ...
                if index == 2:
                    tmp[index1,3]             = 1
                    fixeDelta[index+1,index1,3] = 1 # translate black hair to blond hair
                elif index == 3:
                    tmp[index1,4]             = 1
                    fixeDelta[index+1,index1,4] = 1 # translate blond hair to Brown Hair
                elif index == 4:
                    tmp[index1,3]             = 1
                    fixeDelta[index+1,index1,3] = 1
        fixedtraget.append(tmp)
    return fixedImages,fixeDelta,fixedtraget